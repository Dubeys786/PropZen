...
/// Modal for Viewing Dealer Payment History & Tax Invoices
class DealerPaymentHistoryModal extends StatefulWidget {
  const DealerPaymentHistoryModal({super.key});
  ...
  static void show(BuildContext context) {
    showModalBottomSheet(
      ...
      builder: (_) => const DealerPaymentHistoryModal(),
    );
  }

  @override
  State<DealerPaymentHistoryModal> createState() => _DealerPaymentHistoryModalState();
}

class _DealerPaymentHistoryModalState extends State<DealerPaymentHistoryModal> {
  ...
  Future<void> _loadPayments() async {
    ...
    final list = await _service.getPaymentHistory(dealerId);
    setState(() {
      _payments = list;
      _isLoading = false;
    });
                ...
                Row(
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        Text(
                          'Dealer Billing & Invoices',
                          style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold, color: AppTheme.textPrimary),
                        ),
                        Text(
                          'Verified GST-compliant Razorpay transaction records',
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(LucideIcons.receipt, size: 36, color: Color(0xFF94A3B8)),
            ),
            ...
            Text(
              'Your Starter Plan is active free of charge. Paid upgrades and renewals will appear here with downloadable GST receipts.',
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            Text('Dealer Subscription Tax Invoice', style: GoogleFonts.poppins(fontSize: 15, fontWeight: FontWeight.bold)),
          ],
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _receiptRow('Subscription Item', p.planName),
            _receiptRow('Amount Paid', p.formattedAmount),
            _receiptRow('Provider', 'Razorpay Gateway Verified'),
            _receiptRow('Verification Status', 'ACTIVE SUCCESS ✓'),
            _receiptRow('Payment Reference', p.id),
            _receiptRow('Order Reference', p.orderId),
            _receiptRow('Transaction ID', p.transactionId ?? 'TXN-DLR-${DateTime.now().millisecondsSinceEpoch}'),
            _receiptRow('Invoice Date', p.formattedDate),
          ],
        ),
        actions: [
          TextButton(
